...
Maze::Maze(const int width, const int height, const int startX, const int startY, const int endX, const int endY) {
...
void Maze::setWidthHeight(const int width, const int height, const int startX, const int startY,
...
void Maze::setCell(const int x, const int y, Cell *cell) const { cells_[x][y] = cell; }
...
bool Maze::addWall(const int x, const int y, const bool horizontal, const bool neighborsConnection) const {
    if (x < 0 || x >= width_ || y < 0 || y >= height_) {
        return false;
    }
